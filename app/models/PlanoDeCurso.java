...
public class PlanoDeCurso extends Model{
	...
	public static int MINIMO_CREDITOS_CONCLUIR = 208;
	 ...
	 */
	private void alocaPeriodos (int numeroDePeriodos) {
		if (periodos.size() == 0) {
			for (int i = 1; i<= numeroDePeriodos; i++ ){
				Periodo p = new Periodo(i, MAXIMO_CREDITOS);
				for (Disciplina d: mapaDeDisciplinas.values()) {
					if (d.getPeriodo() == p.getNumero()) {
						try {
	 ...
	 * primeira vez.
	 */
	public void distribuiCaderas(List<Disciplina> disciplinas, int numeroDePeriodos){
		atualizaMapaCadeira(disciplinas);
		alocaPeriodos(numeroDePeriodos);
		atualizaValidadores();
	}
	...
	public Map<String, Disciplina> getMapDisciplinasDisponiveis() {
		...
		for (Disciplina c: mapaDeDisciplinas.values()) {
			if (!alocadas.contains(c) && c.getPeriodo() != -1) {
